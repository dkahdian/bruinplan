<script lang="ts">
	...
	onMount(async () => {
		try {
			const { courses: loadedCourses } = await loadCourses();
	...
	function selectCourse(courseId: string) {
		...
		const urlSafeCourseId = courseId.replace(/\s+/g, '');
	...
	<div class="bg-gradient-to-r from-purple-600 to-pink-600 text-white py-16">
		<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
			...
			<p class="text-xl text-purple-100 mb-8 max-w-2xl mx-auto">
				Find any UCLA course and explore its prerequisite graph to understand what you need to take first.
			</p>
		</div>
	</div>
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
		...
		{:else}
			...
			<div class="bg-white rounded-2xl p-8 shadow-xl mb-8">
				<div class="mb-6">
					<label for="course-search" class="block text-lg font-semibold text-gray-900 mb-2">
						Search for a course
					</label>
					<input
						id="course-search"
						...
						class="w-full px-4 py-3 text-lg border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
						autocomplete="off"
					/>
				</div>
				...
				<div class="bg-white rounded-2xl shadow-xl overflow-hidden">
					<div class="divide-y divide-gray-200">
						{#each searchResults as course}
							<button
								class="w-full px-6 py-4 text-left hover:bg-gray-50 transition-colors group cursor-pointer"
								...
								<div class="flex items-start justify-between">
									<div class="flex-1 min-w-0">
										<div class="flex items-center gap-3 mb-2">
											<span class="text-lg font-semibold text-purple-700 group-hover:text-purple-800">
												{course.id}
											</span>
											{#if course.units}
